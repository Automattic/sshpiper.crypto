...
func (t *handshakeTransport) enterKeyExchange(otherInitPacket []byte) error {
	...
	// about server's supported sign algos where we need to include RSA SSH2.
	// If the server does not provide this message then the client will not
	// attempt to authenticate with any RSA key as the server may not support
	// RSA SSH2 (256 & 512).
	// sshpiperd uses golang's crypto library and we can fix that on server side
	// with ext-info-c SSH extension which is supported by the new ssh clients
	// and send SSH_MSG_EXT_INFO message that contains RSA SSH2 algorithms.
	// More info: RFC 8308, Sections 2.4 and 3.1.
	//
